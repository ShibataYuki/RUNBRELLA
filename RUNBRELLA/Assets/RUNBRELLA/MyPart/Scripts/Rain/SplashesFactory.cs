...
{    
    ...
    void Start()
    {
        ...
        framePerSplash = howManySplashSteps;
    }
    ...
    {                
        ...
        while(true)
        {
            ...
            StartEffect(effect);
            // エフェクトの発生ペースをコントロール
            for(int i = 0; i < framePerSplash; i++)
            {
                yield return null;
            }
        }        
    ...
    void ChangeFramePerSplash()
    {
        // 100% / しぶきの強さの段階　＝　しぶきの強さが変化するパーセントの境
        var BorderParcentage = 100f / howManySplashSteps;
        // 強さ段階の数　- (現在の雨の強さの％ / しぶきの強さの変化するボーダー)
        // 小数点切り捨て ＝「framePerSplash」
        //例)
        // 5 - 19% / 20% = 5
        // 5 - 20% / 20% = 4
        // 5 - 40% / 20% = 3 ...
        framePerSplash = howManySplashSteps - (int)(rain.rainPercentage / BorderParcentage);
        // 5 - 100% / 100% = 0
        // 「framePerSplash」が０だと無限ループになるので範囲制限
        framePerSplash = Mathf.Clamp(framePerSplash, 1, howManySplashSteps);       
    }
